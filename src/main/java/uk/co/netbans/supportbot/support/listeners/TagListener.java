...
public class TagListener extends ListenerAdapter {
        ...
        if (!event.getMessage().getMentionedRoles().isEmpty()) {
            for (Long role : getRoleMentions()) {
                ...
                if (event.getMessage().getMentionedRoles().contains(role1)) {
                    TextChannel textChannel = bot.getJDA().getTextChannelById(bot.getConfig().getConfigValue("mention_channel").getAsLong());
    ...
    private List<Long> getRoleMentions() {
        ...
        JsonObject mentions = bot.getConfig().getConfigValue("mentions");
